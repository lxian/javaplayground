...
public class AvlTree<K extends Comparable<K>> {
    ...
    public Object find(K key) {
        Node<K> found = doFind(key, root, false);
        return found == null ? null : found.value;
    }

    public Node<K> findNearest(K key) {
        return doFind(key, root, true);
    }

    private Node<K> doFind(K key, Node<K> node, boolean nearest) {
        if (node == null) {
            return null;
        } else if (key == node.key || key.compareTo(node.key) == 0) {
            return node;
        } else if (key.compareTo(node.key) < 0) {
            return doFind(key, node.left, nearest);
        } else {
            Node<K> found = doFind(key, node.right, nearest);
            if (found != null) {
                return found;
            } else if (nearest) {
                return node;
